export default {
    ...
    scheduleGenerationInfoIntro:
      "Перед запуском система берёт секции выбранного семестра, преподавателей, доступные аудитории, блокировки аудиторий, подгруппы и предпочтения преподавателей. Уже занятые слоты между пакетами учитываются как запреты.",
    scheduleGenerationGreedyInfo:
      "Greedy — быстрый режим. Он ставит занятия по очереди: сначала лекции, затем практики и лабораторные, проверяя занятость преподавателя, группы, подгруппы и аудитории. После первичной расстановки он дополнительно пытается выровнять нагрузку по дням.",
    scheduleGenerationCpSatInfo:
      "CP-SAT — более точный режим. Он строит математическую модель и ищет лучшее допустимое расписание с учётом ограничений: вместимость, тип аудитории, компьютеры, язык группы, лекция перед практикой/лабораторной, блокировки, лимиты преподавателей и предпочтения.",
    scheduleGenerationHybridInfo:
      "Hybrid сначала пробует CP-SAT. Если решение не найдено или оптимизатор недоступен, система переключается на Greedy.",
    scheduleGenerationInfoNote:
      "CP-SAT может работать дольше, но обычно даёт более качественное расписание. Greedy быстрее и полезен, когда нужно получить результат без долгого ожидания.",
    day: "День",
